package daxapi.teleports;

import daxapi.Filters;
import daxapi.apilib.models.Requirement;
import daxapi.shared.helpers.InterfaceHelper;
import daxapi.shared.helpers.RSItemHelper;
import daxapi.shared.helpers.WorldHelper;
import daxapi.shared.helpers.magic.Spell;
import daxapi.teleports.teleport_utils.TeleportConstants;
...
public enum Teleport {
	...
	RING_OF_WEALTH_MISCELLANIA(
			...
			() -> inMembersWorld() && WearableItemTeleport.has(WearableItemTeleport.RING_OF_WEALTH_FILTER) && Web.methods.clientLocalStorage.getVarpValueAt(359) >= 100,
	...
	private static boolean handleScrollInterface(String regex){
		RSWidget box = Web.methods.interfaces.getComponent(187, 3);
		if(box == null)
			return false;
	...
	private static boolean selectSpell(String spellName, String action){
		if(!Web.methods.game.openTab(InterfaceTab.MAGIC)){
			return false;
		}
