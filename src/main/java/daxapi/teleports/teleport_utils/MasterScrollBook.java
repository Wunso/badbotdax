...
public class MasterScrollBook {
	...
	private static RSWidget getInterface(Teleports teleport){
		if(cache.containsKey(teleport.getName())){
			return Web.methods.interfaces.getComponent(INTERFACE_MASTER,cache.get(teleport.getName()));
		}
	...
	private static String getDefaultTeleportText(){
		RSWidget master = Web.methods.interfaces.getComponent(SELECT_OPTION_MASTER,SELECT_OPTION_CHILD);
		if(master == null)
			return null;
